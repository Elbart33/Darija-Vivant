    ...
    >
      {explanationDarija ? (
        <>
          ...
          {explanationFr && !showFrench && (
            <div className="mt-3">
              <button
                type="button"
                onClick={() => setShowFrench(true)}
                className={`animate-pulseSoft flex w-full items-center justify-between gap-3 rounded-lg border-2 px-4 py-3 text-left transition-colors ${
                  tone === "correction"
          ...
          {explanationFr && showFrench && (
            <p className="mt-3 animate-fadeUp">{renderExplanation(explanationFr, tone)}</p>
          )}
        </>
      ) : (
